    ...
    <div className="page" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div className="main">
        <div className="card">
          ...
          {currentLoad && (
            <div>
              <p>Tracking URL: <a href={currentLoad.trackingUrl} target="_blank" rel="noreferrer">{currentLoad.trackingUrl}</a></p>
              <button onClick={handleNavigate}>🚀 Open Tracking Page</button>
              ...
              {driverLocation && loadStatus === 'Confirmed' && (
                <div>
                  ...
                  <p>Last Update: {new Date(driverLocation.timestamp).toLocaleString()}</p>
                </div>
              )}
            </div>
          )}
