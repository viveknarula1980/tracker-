...
export default function LoadDetails() {
  ...
  const calculateRoute = (destinationIndex = 0) => {
    if (!directionsService || !load?.stops[destinationIndex]) {
    ...
    if (!userLocation) {
      console.log('No user location available, requesting current position...');
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          ...
          (error) => {
            console.error('Geolocation error:', error);
            setError('Location access denied. Please enable location services to use navigation.');
          },
